#!/bin/bash
set -e
echo "Applying: preview original imported PDF in modal..."
...
git add -A
git status
echo "Done. Review the diff, then: git commit -m \"Preview original imported PDF instead of empty mockup\" && git push"
