#!/bin/bash
set -e
echo "Applying: repertoire list view (replaces card grid)..."
...
.list-row{
  display:grid; grid-template-columns:150px 1fr 150px 130px 20px; gap:16px; align-items:center;
...
.list-row:not(.list-row-head) .col-material{font-size:12px; color:var(--ink-soft);}
...
git add -A
git status
echo "Done. Review the diff, then: git commit -m \"Replace card grid with list view in repertoire\" && git push"
